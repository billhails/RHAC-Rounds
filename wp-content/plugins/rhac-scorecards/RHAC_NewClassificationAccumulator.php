...
class RHAC_NewClassificationAccumulatorLeaf extends RHAC_AccumulatorLeaf {
    ...
    public function accept($row) {
        if ($row['reassessment'] == "age_group") {
            ...
            $cfn = $row['classification'];
            if ($this->addAndReportCount($cfn) == 3) {
                $this->remember($cfn, $row['date']);
                if ($this->better($cfn, $this->classification_this_season)) {
                    $this->classification_this_season = $cfn;
                }
                if ($this->better($cfn, $this->current_classification)) {
                    $this->current_classification = $cfn;
                    $this->noteClassificationChange($row);
                }
                elseif ($this->equal($cfn, $this->current_classification)) {
    ...
    private function remember($classification, $date) {
    ...
    private function noteClassificationChange($row) {
        $this->proposed_changes[$row['scorecard_id']] = $this->current_classification;
        $this->current_db_values[$row['scorecard_id']] = $row['new_classification'];
    }
